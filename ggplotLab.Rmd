...
```{r}
ht <- ggplot (heights, aes(x = height)) 
ht + geom_histogram(fill = "blue")
```

Change up the binwidth and see how the plots change. Try 1, 5, 10, and 20

```{r}
ht + geom_histogram(binwidth = 3)
...
```{r}
ggplot(heights, aes(x = height, fill = sex)) + geom_density(alpha = 0.3)
...
```{r}
ggplot(heights, aes(x = height, fill = sex)) + geom_boxplot()
```
...
```{R}
heights %>% group_by(sex)%>% summarise( max_height = max(height), min_height = min(height))
...
```{R}
heights %>% group_by(sex)%>% summarise( mean_height = mean(height))%>%
ggplot(aes(x= sex, y = mean_height, fill= sex)) + geom_col()
```
